...
Generated by `scripts/perf.mjs` on 2026-08-12T15:42:47.021Z.
...
|---|---|---|---|---|---|
| `/briefing` | 30 | 25.3 | 35.4 | 37.3 | 200x30 |
| `/dashboard` | 30 | 14.5 | 20.9 | 31.0 | 200x30 |
| `/sites` | 30 | 8.9 | 13.7 | 17.9 | 200x30 |
| `/directors` | 30 | 9.1 | 12.9 | 18.2 | 200x30 |
| `/vendors` | 30 | 8.7 | 12.2 | 13.1 | 200x30 |
| `/contracts` | 30 | 13.0 | 16.5 | 18.9 | 200x30 |
| `/emails` | 30 | 7.7 | 10.6 | 12.8 | 200x30 |
| `/search?q=fire` | 30 | 10.7 | 15.5 | 21.6 | 200x30 |
| `/calendar` | 30 | 7.5 | 10.7 | 12.1 | 200x30 |
| `/notes` | 30 | 8.3 | 11.8 | 12.0 | 200x30 |

### 2. Concurrency (20 parallel requests)

| Route | Concurrency | Wall time (ms) | p50 (ms) | p95 (ms) | max (ms) | Statuses |
|---|---|---|---|---|---|---|
| `/briefing` | 20 | 185.8 | 180.1 | 183.6 | 184.1 | 200x20 |
| `/dashboard` | 20 | 157.7 | 155.7 | 156.5 | 156.8 | 200x20 |

### 3. Heavier read: `/api/export/briefing` (10 requests)

| Route | n | p50 (ms) | p95 (ms) | max (ms) | Statuses |
|---|---|---|---|---|---|
| `/api/export/briefing` | 10 | 20.1 | 28.3 | 28.3 | 200x10 |

### 4. After data growth (+200 ActivityEvent, +100 Note; totals: 200 events, 100 notes)

| Route | n | p50 (ms) | p95 (ms) | max (ms) | Statuses |
|---|---|---|---|---|---|
| `/briefing` | 30 | 22.0 | 26.5 | 26.7 | 200x30 |
| `/search?q=note` | 30 | 10.5 | 12.8 | 13.1 | 200x30 |

## Budget verdicts

| Budget | Result | Evidence |
|---|---|---|
| p95 <= 1500 ms for every page at seed scale | PASS | worst page p95 = 35.4 ms (/briefing) |
...
| /briefing p95 <= 2500 ms after data growth (+200 events, +100 notes) | PASS | /briefing p95 after growth = 26.5 ms |
