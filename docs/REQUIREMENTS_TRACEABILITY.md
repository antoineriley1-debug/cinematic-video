...
| R33.2 | Plaud API sync | DEFERRED by user decision (2026-08-12). Official docs reviewed: device sync needs a mobile app (Embedded SDK); the server-side path is Plaud's Transcription API (partner/user token flow at platform-us.plaud.ai) to auto-transcribe our drag-and-dropped audio. Accurate plan + endpoints documented in `services/plaudSync.ts`; needs client_id+client_secret from portal.plaud.ai, the Transcription API spec page, and network access | design documented; dormant scaffolding tested | IMPLEMENTED |
