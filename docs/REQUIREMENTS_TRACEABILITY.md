...
| R33.2 | Plaud Transcription API integration | `services/plaudClient.ts`: exact documented auth flow (Basic client_id:secret → Partner Token, cached; Bearer → User Token with 6-120 char user_id) + transcription submit via signed public file URL (`lib/signedUrl.ts`, `/api/public-files/[id]`) → transcript auto-attached + analyzed; 'Transcribe with Plaud' button on audio recordings; admin status row. Transcription endpoint path configurable (PLAUD_TRANSCRIBE_PATH) pending Plaud's API reference page | T:plaud-transcription.test.ts (12 tests: auth headers/caching/user_id constraints, transcript extraction incl. speaker segments, pending jobs, end-to-end upload→transcribe→analyze, signed-URL security) — live call needs portal.plaud.ai client_id+secret and network/production access | IMPLEMENTED |
